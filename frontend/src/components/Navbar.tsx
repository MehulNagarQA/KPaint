    ...
    <nav className="fixed w-full z-50 glass border-b border-white/10 px-6 py-4 transition-all top-0">
      <div className="max-w-7xl mx-auto flex justify-between items-center">
        ...
        <Link to="/" className="flex items-center gap-2 group">
          <Palette className="w-8 h-8 text-[#1877F2] group-hover:rotate-12 transition-transform" />
        ...
        <div className="flex items-center gap-6">
          <Link to="/cart" className="relative group p-2">
            <ShoppingCart className="w-6 h-6 text-gray-300 group-hover:text-[#1877F2] transition-colors" />
            {cartItemsCount > 0 && (
              <span className="absolute top-0 right-0 bg-[#1877F2] text-white text-xs font-bold rounded-full w-5 h-5 flex items-center justify-center -mt-1 -mr-1">
                {cartItemsCount}
              </span>
            )}
          </Link>
          ...
          {isAuthenticated ? (
            <div className="group relative">
              <button className="flex items-center gap-2 text-gray-300 hover:text-[#1877F2] transition-colors font-medium">
              ...
              <div className="absolute right-0 mt-2 w-48 rounded-xl bg-[#0a0a0a] border border-[#1f1f1f] py-2 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all glass shadow-xl shadow-black/50">
                ...
                {user?.role === 'admin' && (
                  <Link to="/admin" className="flex items-center gap-3 px-4 py-3 text-sm text-gray-300 hover:bg-white/5 hover:text-[#1877F2] transition-colors">
